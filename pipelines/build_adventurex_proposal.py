...
SOURCE = ROOT / "AdventureX_沙地精准灌溉机器人_最终方案.md"
IMAGE = ROOT / "output" / "images" / "沙地精准灌溉机器人_模型图.png"
OUTPUT = ROOT / "output" / "AdventureX_沙地精准灌溉机器人_最终方案.docx"
...
def add_header_footer(section, first: bool = False) -> None:
    ...
    r1 = p.add_run("ROOTDROP  /  ADVENTUREX 2026")
    r1.bold = True
    r1.font.size = Pt(8.5)
    r1.font.color.rgb = RGBColor.from_string(NAVY)
    set_east_asia(r1)
    r2 = p.add_run("    看得见的地下灌溉，验得出的每一滴")
    r2.font.size = Pt(8.5)
...
def add_cover(doc: Document) -> None:
    ...
    tag = p.add_run("ADVENTUREX 2026  ·  FINAL v1.0")
    tag.bold = True
    tag.font.size = Pt(10)
    ...
    run = title.add_run("沙脉 RootDrop")
    set_east_asia(run)

    sub = doc.add_paragraph(style="Subtitle")
    sub.alignment = WD_ALIGN_PARAGRAPH.LEFT
    add_inline(sub, "看得见的地下灌溉，验得出的每一滴")

    meta = doc.add_paragraph()
    meta.paragraph_format.space_after = Pt(10)
    add_inline(meta, "AdventureX 2026 冲奖执行方案  |  四人团队  |  72 小时  |  2026-07-14", size=9.5, color=MUTED)

    pic_p = doc.add_paragraph()
    pic_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    pic_p.paragraph_format.space_after = Pt(2)
    shape = pic_p.add_run().add_picture(str(IMAGE), width=Inches(5.7))
    set_alt_text(shape, "沙脉 RootDrop 概念模型", "四轮底盘、RDK X5、STM32F407、侧置短行程探针、低位水箱和透明根区沙箱的三天可制造概念图")

    cap = doc.add_paragraph(style="Caption")
    cap.paragraph_format.space_after = Pt(8)
    add_inline(cap, "三天可制造布局概念图 · 实际尺寸以现有底盘、滑台和 0.3–0.5 L 水箱实测为准")
    ...
    add_inline(cp, "最终拍板：1×RDK X5 上位智能 + 1×STM32F407 下位安全；硬地短路线、侧置双深度探针、定量给水、侧视湿润团复核与证据回执。", size=10.5, color=NAVY, bold=True)
    ...
    add_inline(p, "目标：优先冲击地瓜机器人合作方赛道一等奖；其他奖项以 2026 正式题面与资格为准", size=9, color=MUTED)
...
def parse_body(doc: Document, lines: list[str]) -> None:
    ...
    while i < len(lines):
        ...
        elif line.startswith("### 6.2"):
            doc.add_page_break()

        if line.startswith("### 11.2") and not landscape:
            add_landscape_section(doc)
            landscape = True
        elif line.startswith("### 11.3") and landscape:
            add_portrait_section(doc)
            landscape = False
        ...
        if number_match:
            ...
            prefix.font.color.rgb = RGBColor.from_string(TEXT)
            set_east_asia(prefix)
...
def build() -> Path:
    ...
    doc.core_properties.title = "沙脉 RootDrop：AdventureX 2026 冲奖执行方案"
    doc.core_properties.subject = "RDK X5 + STM32F407 沙地精准灌溉机器人的 72 小时执行方案"
    doc.core_properties.author = "荧光具身智研 / RootDrop 团队"
    doc.core_properties.keywords = "AdventureX, RDK X5, STM32F407, 灌溉机器人, 具身智能"
    doc.core_properties.comments = "Final v1.0 · 2026-07-14"
