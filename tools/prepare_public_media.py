...
redacts screens/labels, drops audio, and cuts the demo before people enter frame.
"""
...
def write_video(source: Path, public: str, start: float, end: float, redact_label: bool = True) -> dict[str, str | int | float]:
    ...
    while frame_index < max_frames:
        ...
        if not ok:
            break
        if redact_label:
            # The collection bottle moves only slightly in this fixed technical cut.
            # Redact its small QR/text and barcode zones without hiding the water flow.
            for box in ((0.32, 0.64, 0.72, 0.82), (0.30, 0.75, 0.82, 1.00)):
                x0, y0, x1, y1 = px_box(box, width, height)
                frame[y0:y1, x0:x1] = cv2.GaussianBlur(frame[y0:y1, x0:x1], (41, 41), 0)
        writer.write(frame)
        frame_index += 1
    writer.release()
    cap.release()
    return {
        ...
        "rights_basis": PUBLIC_RIGHTS_BASIS,
        "transform": f"trim {start:.1f}-{end:.1f}s; audio removed; H.264/yuv420p; bottle label redacted; container metadata discarded",
    }
...
def make_poster(source: Path) -> dict[str, str | int]:
    ...
    return {
        ...
        "rights_basis": PUBLIC_RIGHTS_BASIS,
        "transform": "frame at 27.0s; bottle label redacted; metadata removed",
    }
...
def main() -> None:
    ...
    manifest_json.write_text(json.dumps({"schema": "rootscope.public-media.v1", "assets": records}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
